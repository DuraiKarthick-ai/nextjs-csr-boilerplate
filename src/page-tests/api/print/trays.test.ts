/**
 * Unit tests for POST /api/print/trays. ecsWsCall is mocked.
 */

jest.mock("../../../lib/ecsWebSocket", () => ({ ecsWsCall: jest.fn() }));

import type { NextApiRequest, NextApiResponse } from "next";
import handler from "../../../pages/api/print/trays";
...
describe("POST /api/print/trays", () => {
  ...
  it("returns 400 when sessionID or printer is missing", async () => {
    const res = mockRes();
    await handler(req("POST", { sessionID: "s1" }), res);
    expect(res._status).toBe(HTTP_STATUS.BAD_REQUEST);
  });

  it("returns 200 with the trays list", async () => {
    wsCall.mockResolvedValue({ trays: ["T1"] });
    const res = mockRes();
    await handler(req("POST", { sessionID: "s1", printer: "P1" }), res);
    expect(res._status).toBe(HTTP_STATUS.OK);
    expect(res._json.trays).toEqual(["T1"]);
  });
