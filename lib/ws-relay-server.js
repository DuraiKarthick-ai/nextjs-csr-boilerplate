...
class PrintRelayServer {
  ...
  _registerRelay(storeId, ws) {
    ...
    ws.on("message", (data) => {
      try {
        ...
        if ((msg.action === "http-response" || msg.action === "ws-response") && msg.id) {
  ...
  async sendRequest(url, method, body, storeId = "default", headers) {
    const ws = this.relays.get(storeId);
    if (!ws || ws.readyState !== WebSocket.OPEN) {
  ...
  async sendWsRequest(url, payload, storeId = "default", timeoutMs = RELAY_TIMEOUT_MS) {
    const ws = this.relays.get(storeId);
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      throw new Error(`No relay connected for store "${storeId}"`);
    }
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        reject(new Error(`Relay WS request timed out after ${timeoutMs}ms`));
      }, timeoutMs + 1000);

      this.pending.set(id, {
        resolve: (res) => {
          if (res.success === false) {
            reject(new Error(res.error || "Relay WS request failed"));
            return;
          }
          resolve({ statusCode: res.statusCode || 200, body: res.body || "{}" });
        },
        reject,
        timer,
      });
...
// Use globalThis so that webpack-bundled API routes and server.js (native require)
// always reference the exact same instance. Without this, Next.js webpack creates
// a separate module scope and the API route gets a "fresh" relay that was never attached.
if (!globalThis.__printRelaySingleton) {
  globalThis.__printRelaySingleton = new PrintRelayServer();
}
const printRelay = globalThis.__printRelaySingleton;
